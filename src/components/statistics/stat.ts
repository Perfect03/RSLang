...
export const createStatistic = () => {
    ...
    const main_stat_accuracy_progress_circle = document.createElement('div');
    main_stat_accuracy_progress_circle.classList.add('main_stat_accuracy_progress_circle');
    main_stat_accuracy.append(main_stat_accuracy_progress_circle);
    main_stat_accuracy_progress_circle.dataset.percent = '0';

    const accuracy_circle_progress = document.createElement('div');
    accuracy_circle_progress.classList.add('accuracy_circle_progress');
    main_stat_accuracy_progress_circle.append(accuracy_circle_progress);
    ...
    main_stat_accuracy_progress_circle.append(accuracy_circle_progress_percents);
    ...
    function createCircle() {
        const progress_circle = document.querySelector('.main_stat_accuracy_progress_circle') as HTMLElement;
        const percent = parseInt(progress_circle.dataset.percent!);
        const percent_line = (360 * percent) / 100;

        if (percent > 50) {
            progress_circle?.classList.add('gt_50');
        }
        (document.querySelector('.accuracy_circle_progress_fill') as HTMLElement).style.transform =
            'rotate(' + percent_line + 'percent_line)';
        (document.querySelector('.ppc_percents_span') as HTMLElement).innerText = percent + '%';
    }
